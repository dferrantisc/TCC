...
module.exports = {
    ...
    async findBySorteio(sorteio) {
        try {
            ...
            const sorteios = await sequelize.query(
                'select * from sorteio_cliente sc inner join usuariocliente u on u.iduc = sc.iduc where sc.idsort = :sorteio', { replacements: { sorteio }, type: sequelize.QueryTypes.SELECT }
            )
            return sorteios
        } catch (error) {
            console.log(error)
    ...
    async update(id_cliente, id_sorteio) {
        try {
            const updatedSorteioCliente = await entdds.updatedSorteioCliente.update({ id_cliente, id_sorteio }, {
                where: {
                    idsort: id_sorteio,
                    iduc: id_cliente
                },
